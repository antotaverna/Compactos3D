...
c ------galaxias
        ...
        real den_aisl,den_a,cascaron,pi
...
c------------ FILENAMES -------------------------------   
        if(ncosmo.eq.1)then
          ...
          filecosmo='../Mill_I/red2dis_lcdm.dat-mill_wmap1'
           write(filein,'("guo_11/tablaglx.dat")')
           write(fileout1,'("guo_11/galx_gru.dat")')
           write(fileout2,'("guo_11/tablagru.dat")')
           write(fileout3,'("guo_11/perfiles.dat")')
           write(fileout4,'("guo_11/file_dim.dat")')
        endif
        !----------------
        if(ncosmo.eq.2)then
          lbox=500.
          ...
          filecosmo='../Mill_I/red2dis_lcdm.dat-mill_wmap7'
           write(filein,'("guo_13/tablaglx.dat")')
           write(fileout1,'("guo_13/galx_gru.dat")')
           write(fileout2,'("guo_13/tablagru.dat")')
           write(fileout3,'("guo_13/perfiles.dat")')
           write(fileout4,'("guo_13/file_dim.dat")')
        endif
        !----------------
        if(ncosmo.eq.3)then
          lbox=480.279
          ...
          filecosmo='../Mill_I/red2dis_lcdm.dat-mill_planck'
           write(filein,'("hen_15/tablaglx.dat")')
           write(fileout1,'("hen_15/galx_gru.dat")')
           write(fileout2,'("hen_15/tablagru.dat")')
           write(fileout3,'("hen_15/perfiles.dat")')
           write(fileout4,'("hen_15/file_dim.dat")')
        endif
        !----------------
        if(ncosmo.eq.4)then
          lbox=100.
          ...
          filecosmo='../Mill_II/red2dis_lcdm.dat-mill_wmap1'
           write(filein,'("guo_II/tablaglx.dat")')
           write(fileout1,'("guo_II/galx_gru_0.07.dat")')
           write(fileout2,'("guo_II/tablagru_0.07.dat")')
           write(fileout3,'("guo_II/perfiles_0.07.dat")')
           write(fileout4,'("guo_II/file_dim_0.07.dat")')
        endif
        !----------------
        if(ncosmo.eq.5)then
          lbox=100.
          h0=0.73
          filecosmo='../Mill_II/red2dis_lcdm.dat-mill_wmap1'
           write(filein,'("guo_II_16/tablaglx.dat")')
           write(fileout1,'("guo_II_16/galx_gru.dat")')
           write(fileout2,'("guo_II_16/tablagru.dat")')
           write(fileout3,'("guo_II_16/perfiles.dat")')
           write(fileout4,'("guo_II_16/file_dim.dat")')
        endif
        !----------------
        if(ncosmo.eq.7)then
          lbox=480.279
          h0=0.673
          filecosmo='../Mill_I/red2dis_lcdm.dat-mill_planck'
           write(filein,'("hen_20/tablaglx.dat")')
           write(fileout1,'("hen_20/galx_gru.dat")')
           write(fileout2,'("hen_20/tablagru.dat")')
           write(fileout3,'("hen_20/perfiles.dat")')
           write(fileout4,'("hen_20/file_dim.dat")')
        endif
        !----------------
        if(ncosmo.eq.8)then
          lbox=480.279
          h0=0.673
          filecosmo='../Mill_I/red2dis_lcdm.dat-mill_planck'
           write(filein,'("ayr_21/tablaglx.dat")')
           write(fileout1,'("ayr_21/galx_gru.dat")')
           write(fileout2,'("ayr_21/tablagru.dat")')
           write(fileout3,'("ayr_21/perfiles.dat")')
           write(fileout4,'("ayr_21/file_dim.dat")')
        endif
        !----------------
...
c        open(33,file=fileout3,status='unknown')
...
! Lectura de la tabla1
        ...
        rl=float(ngrid)
        fac=rl/lbox
        if(ncosmo.eq.4)masslim=0.07 !0.007
        do i=1,nmax
                call percent(i,nmax,'lectura')
     ...
     &                    stmasst,bmasst,igrut,inumt
                ...
                !if(stmasst.lt.masslim)goto 21

                 suma_stm=suma_stm+stmasst
                 !guardo pos,rmag,igru para glx en gru + campo
                 igru_new(i)=igrut
                 rr_new(1,i)=x*fac
                 rr_new(2,i)=y*fac
                 rr_new(3,i)=z*fac
                 ...
                 if(ncosmo.eq.3.or.ncosmo.eq.7.or.ncosmo.eq.8)then
                       kmag(it)=kmagt-5.*log10(h0) !Hen_15
                       jmag(it)=jmagt-5.*log10(h0) !Hen_15
                 endif
                 stmass(it)=stmasst
                 bmass(it)=bmasst
...
c
 ...
 11     continue
        ...
        write(*,*)'numero de galaxias en grupos sobredensos nmi>4',ngal
...
c             end if
 ...
 87            i=ll(i)               
                        ...
                        zzmag(ii)=zmag(i)
                        if(ncosmo.eq.3.or.ncosmo.eq.7.or.ncosmo.eq.8)then
                           kkmag(ii)=kmag(i) !Hen_15
                           jjmag(ii)=jmag(i) !Hen_15
                        endif   
                        ggID(ii)=gID(i)
                        iigru(ii)=igru(i)
                        iinum(ii)=inum(i)
...
c                call perfil2(0,fac,nbin,ngrid,rvir_bi,lbox,xcm,ycm,zcm,rr_new,stm_new,
c     2          j,igru_new,lirstn,lgn,xbin,dens_new,dmax_x)
c
                !perfil glx miembro
c                call perfil2(2,fac,nbin,ngrid,rvir_bi,lbox,xcm,ycm,zcm,rr_new,stm_new,
c     2          j,igru_new,lirstn,lgn,xbin,dens_mie,dmax)
c
                !perfil glx campo
c                call perfil2(1,fac,nbin,ngrid,rvir_bi,lbox,xcm,ycm,zcm,rr_new,stm_new,
c     2          j,igru_new,lirstn,lgn,xbin,dens_cmp,dmax_x)

                !perfil glx otros grupos
c                call perfil2(3,fac,nbin,ngrid,rvir_bi,lbox,xcm,ycm,zcm,rr_new,stm_new,
c     2          j,igru_new,lirstn,lgn,xbin,dens_gru,dmax_x)
...
c                write(33,78)j,(xbin(k),k=1,nbin),(dens_cmp(k),k=1,nbin),(dens_new(k),k=1,nbin),
c     & (dens_mie(k),k=1,nbin),(dens_gru(k),k=1,nbin)
 ...
 12     end do
        ...
        write(*,*)'grupos difusos nmi>=3',ngfin

 23     format(i18,1x,7(f16.8,1x),5(f14.8,1x),2(e12.4,1x),2(i8,1x))
 24     format(i18,1x,7(f16.8,1x),7(f14.8,1x),2(e12.4,1x),2(i8,1x))

 25     format(i18,1x,7(f14.8,1x),5(f9.5,1x),2(e12.4,1x),2(i8,1x),i8,1x,2(f9.5,1x))
 26     format(i18,1x,7(f14.8,1x),7(f9.5,1x),2(e12.4,1x),2(i8,1x),i8,1x,2(f9.5,1x))

 77     format(i8,1x,3(f13.8,1x),3(f12.5,1x),f10.3,1x,i8
     &         ,1x,f7.4,1x,e12.4,1x,3(f9.5,1x),i8)
