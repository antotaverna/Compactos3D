...
int main(int argc, char **argv)
{
  ...
  if (argc == 2){
   redshift = atoi(argv[1]); // use command line parameter; // use default from ANAL_PARAMS.H
  }else{
 ...
 if(!(pf1=fopen(filename1,"r")))
  ...
  nsal         = malloc(28*sizeof(int));
  nsal2        = malloc(28*sizeof(int));
  bz           = malloc(28*sizeof(float));
  for(n=0;n<28;n++)
  ...
  nobj=nsal[redshift];
      printf("%d\n",nobj);
   sprintf(filename1, "z%i/repetidas09.dat-%i", redshift,redshift);
  if(!(pf1=fopen(filename1,"r")))
  {
    printf("can't open file `%s`\n",filename1);
    exit(0);
  ...
  nrep=nsal2[redshift]; // no hay repetidas
      printf("%d\n",nrep);
  ...
  fclose(pf1);
   ...
   sprintf(filename, "z%i/out09.dat-%i", redshift, redshift);
...
/*	printf("%f %f %f\n",x[ncont],y[ncont],z[ncont]);*/
  ...
  r0 = bz[redshift]*rlbox/pow((float)nobj,0.33333);/*0.14 para identificar halos de galaxias*/
...
//  pf=fopen("fof.dat","w+");
  sprintf(filename, "z%i/fof09.dat", redshift);
  pf=fopen(filename,"w+");
  for( i = 1 ; i < nobj+1; i++ )
